...
train_data = open(
    "/home/linhw/qianweinan/TextClassification/data/train.txt",
    "r", encoding='utf-8'
).read()
dev_data = open(
    "/home/linhw/qianweinan/TextClassification/data/dev.txt",
    "r", encoding='utf-8'
).read()
test_data = open(
    "/home/linhw/qianweinan/TextClassification/data/test.txt",
    "r", encoding='utf-8'
).read()
...
embedding = nn.Embedding(len(dic),128).cuda()
train_feature = embedding(torch.LongTensor(train_coding).cuda())
dev_feature = embedding(torch.LongTensor(dev_coding).cuda())
test_feature = embedding(torch.LongTensor(test_coding).cuda())
train_set = TensorDataset(train_feature,train_label)
dev_set = TensorDataset(dev_feature,dev_label)
